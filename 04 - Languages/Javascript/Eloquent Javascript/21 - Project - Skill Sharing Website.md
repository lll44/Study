...
A _router_ is a component that helps dispatch a request to the function that can handle it. You can tell the router, for example, that `PUT` requests with a path that matches the regular expression `/^\/talks\/([^\/]+)$/` (`/talks/` followed by a talk title) can be handled by a given function. In addition, it can help extract the meaningful parts of the path (in this case the talk title), wrapped in parentheses in the regular expression, and pass them to the handler function.
...
This uses a similar convention as the file server from the [previous chapter](https://eloquentjavascript.net/20_node.html) for responses—handlers return promises that resolve to objects describing the response. It wraps the server in an object that also holds its state.
...
The `updated` method, which we will define [later](https://eloquentjavascript.net/21_skillsharing.html#updated), notifies waiting long polling requests about the change.
...
The `elt` function used to construct DOM elements is the one we used in [Chapter 19](https://eloquentjavascript.net/19_paint.html).
