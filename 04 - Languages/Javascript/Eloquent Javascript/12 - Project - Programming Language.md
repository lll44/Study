...
Contrast this to the parser we wrote for the configuration file format in [Chapter 9](https://eloquentjavascript.net/09_regexp.html#ini), which had a simple structure: it split the input into lines and handled those lines one at a time. There were only a few simple forms that a line was allowed to have.
...
We use plain JavaScript function values to represent Egg’s function values. We will come back to this [later](https://eloquentjavascript.net/12_language.html#egg_fun), when the special form called `fun` is defined.
